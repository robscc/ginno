...
│ │ ▾ Agents（在场指示）  │ │ │   用户气泡（右） / Agent 气泡（左：文本·思考·工具·卡片·引用·流程）│ │
│ │                      │ │ │   权限确认条（出现时，置于输入框上方）                            │ │
...
| 块 | 说明 | 状态 |
|---|---|---|
| 文本 | Agent 的自然语言，流式追加，末尾有光标动画 | ✅ |
| 思考 thinking | 模型的思考内容（斜体灰字），来自 `thinking`/`reasoning_content` | ✅ |
| 工具调用 tool | `tool · <name>` + 结果预览；执行中显示转圈 `name…` | ✅ |
