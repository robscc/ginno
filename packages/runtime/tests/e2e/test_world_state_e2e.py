...
def test_usage_events_accumulate(create_session, ws_conv):
    ...
    # (cached portions included) and passes through extraction unchanged
    # (usage-stats-design.md §3.5).
    ...
    assert u2[0]["session"]["input_tokens"] == 100 + 150
    assert u2[0]["session"]["cache_read_tokens"] == 130
    assert u2[0]["session"]["cache_creation_tokens"] == 5
    # Hit ratio uses whole-prompt input as denominator (never > 100%).
    assert u2[0]["cache_hit_ratio"] == round(130 / (100 + 150), 4)
