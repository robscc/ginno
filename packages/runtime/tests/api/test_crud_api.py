...
def test_agent_create_update_delete(client):
    r = client.post("/api/agents", json={"id": "qa", "name": "QA"}).json()
    assert r["ok"] and r["agent"]["id"] == "qa"
    r = client.put("/api/agents/qa", json={"name": "QA2"}).json()
    assert r["agent"]["name"] == "QA2"
    assert client.delete("/api/agents/qa").json()["ok"] is True


def test_agent_duplicate_returns_error(client):
    r = client.post("/api/agents", json={"id": "dev", "name": "dup"}).json()
...
def test_todo_crud(client):
    before = len(client.get("/api/todos").json())
    todo = client.post("/api/todos", json={"title": "API task", "priority": "high"}).json()["todo"]
    assert todo["id"]
    assert len(client.get("/api/todos").json()) == before + 1
    upd = client.patch(f"/api/todos/{todo['id']}", json={"done": True}).json()
    assert upd["todo"]["done"] is True
    assert client.delete(f"/api/todos/{todo['id']}").json()["ok"] is True


def test_todo_create_requires_title(client):
    r = client.post("/api/todos", json={"title": ""}).json()
    assert r["ok"] is False


def test_todo_patch_unknown(client):
    r = client.patch("/api/todos/nope", json={"done": True}).json()
    assert r["ok"] is False
...
def test_workflow_create_and_delete(client):
    ...
    assert client.delete(f"/api/workflows/{wf['id']}").json()["ok"] is True


def test_workflow_runs_list(client):
    assert isinstance(client.get("/api/workflow_runs").json(), list)
...
def test_providers_put_and_default(client):
    ...
    # round-trips
    assert client.get("/api/providers").json()["providers"]["custom"]["enabled"] is True


def test_provider_verify_endpoint_never_500s(client):
    r = client.post("/api/providers/does-not-exist/verify")
...
def test_skill_create_list_body_delete(client):
    body = "---\nname: hello\ndescription: say hi\n---\n# Body\nSay hello.\n"
    assert client.post("/api/skills", json={"name": "hello", "body": body}).json()["ok"] is True
    names = {s["name"] for s in client.get("/api/skills").json()}
    assert "hello" in names
    assert "Say hello." in client.get("/api/skills/hello/body").json()["body"]
    assert client.delete("/api/skills/hello").json()["ok"] is True


def test_skill_create_requires_name(client):
    r = client.post("/api/skills", json={"name": "", "body": "x"}).json()
    assert r["ok"] is False


# --------------------------- mcp / kb --------------------------- #
def test_mcp_config_get_put(client):
    assert client.get("/api/mcp/config").json() == {"mcpServers": {}}
    cfg = {"mcpServers": {"vault": {"transport": "stdio", "command": "x"}}}
    assert client.put("/api/mcp", json=cfg).json()["ok"] is True
...
def test_kb_servers_empty(client):
    assert client.get("/api/kb/servers").json() == []
