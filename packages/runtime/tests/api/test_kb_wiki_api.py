...
def test_wiki_search(client, kb_vault):
    r = client.get("/api/kb/wiki/search?q=权限节点").json()
    assert r["ok"] is True
...
def test_wiki_search_by_tag(client, kb_vault):
    r = client.get("/api/kb/wiki/search?tag=cooking").json()
    assert [x["title"] for x in r["results"]] == ["红烧肉做法"]


def test_wiki_list(client, kb_vault):
    r = client.get("/api/kb/wiki/list").json()
    assert r["ok"] is True
...
def test_wiki_stats(client, kb_vault):
    r = client.get("/api/kb/wiki/stats").json()
    assert r["ok"] is True
...
def test_wiki_endpoints_guard_when_disabled(client):
    ...
    assert client.get("/api/kb/wiki/search?q=x").json()["ok"] is False
    assert client.get("/api/kb/wiki/list").json()["ok"] is False
    assert client.get("/api/kb/wiki/stats").json()["ok"] is False
    assert client.post("/api/kb/wiki/index").json()["ok"] is False


def test_wiki_put_config(client, isolated_home):
    vault = isolated_home / "does_not_exist_yet"
    r = client.put("/api/kb/wiki/config", json={"enabled": True, "vault_path": str(vault)}).json()
    assert r["ok"] is True
