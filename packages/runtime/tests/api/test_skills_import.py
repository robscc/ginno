...
def test_import_dir_copies_skills_and_extras(isolated_home, client, tmp_path):
    ...
    r = client.post("/api/skills/import-dir", json={"path": str(src)}).json()
    assert r["ok"] is True
...
def test_import_dir_skip_existing_then_overwrite(isolated_home, client, tmp_path):
    ...
    r1 = client.post("/api/skills/import-dir", json={"path": str(src)}).json()
    ...
    r2 = client.post("/api/skills/import-dir", json={"path": str(src)}).json()
    ...
    r3 = client.post("/api/skills/import-dir", json={"path": str(src), "overwrite": True}).json()
...
def test_import_dir_invalid_path(client):
    assert client.post("/api/skills/import-dir", json={"path": ""}).json()["ok"] is False
    assert client.post("/api/skills/import-dir", json={"path": "/no/such/dir/xyz"}).json()["ok"] is False
...
def test_import_dir_single_skill_dir(isolated_home, client, tmp_path):
    ...
    r = client.post("/api/skills/import-dir", json={"path": str(tmp_path / "solo")}).json()
