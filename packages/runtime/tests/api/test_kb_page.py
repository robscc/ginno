...
def test_page_create_read_update(client, cfg_vault):
    raw = "---\ntitle: A\ntags: [x]\n---\n# A\n\nhello [[B]] and [[C|see]]\n"
    c = client.post("/kb/wiki/page", json={"path": "notes/a.md", "raw": raw}).json()
    assert c["ok"] is True, c

    g = client.get("/kb/wiki/page?path=notes/a.md").json()
    ...
    # resolve by title
    gt = client.get("/kb/wiki/page?title=A").json()
    assert gt["exists"] is True and gt["path"] == "notes/a.md"

    # dangling link → create-able stub
    gd = client.get("/kb/wiki/page?title=NoSuchPage").json()
    assert gd["ok"] is True and gd["exists"] is False

    # update overwrites + refreshes the index
    u = client.put("/kb/wiki/page", json={"path": "notes/a.md", "raw": "---\ntitle: A2\n---\n# A2\n\nworld\n"}).json()
    assert u["ok"] is True
    g2 = client.get("/kb/wiki/page?path=notes/a.md").json()
...
def test_page_post_refuses_overwrite(client, cfg_vault):
    client.post("/kb/wiki/page", json={"path": "n.md", "raw": "# n\n"})
    r = client.post("/kb/wiki/page", json={"path": "n.md", "raw": "# n2\n"}).json()
    assert r["ok"] is False


def test_page_path_safety(client, cfg_vault):
    # must stay inside the vault and be a markdown file
    assert client.put("/kb/wiki/page", json={"path": "../escape.md", "raw": "x"}).json()["ok"] is False
    assert client.post("/kb/wiki/page", json={"path": "../../etc/x.md", "raw": "x"}).json()["ok"] is False
    assert client.put("/kb/wiki/page", json={"path": "notes/a.txt", "raw": "x"}).json()["ok"] is False
    assert client.put("/kb/wiki/page", json={"path": "", "raw": "x"}).json()["ok"] is False
